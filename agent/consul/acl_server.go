...
func (s *Server) canUpgradeToNewACLs(isLeader bool) bool {
	...
	if !s.InACLDatacenter() {
		...
		if mode != structs.ACLModeEnabled || !foundServers {
			s.logger.Printf("[INFO] acl: Cannot upgrade to new ACLs, servers in acl datacenter have not upgraded - found servers: %t, mode: %s", foundServers, mode)
			return false
		}
	}
	...
	if isLeader {
		...
		s.logger.Printf("[INFO] acl: Cannot upgrade to new ACLs, servers in local datacenter do not support new ACLs")
	} else {
		if leaderMode == structs.ACLModeEnabled {
			return true
		}
		s.logger.Printf("[INFO] acl: Cannot upgrade to new ACLs, leader has not upgraded yet")
	}
